...
function Mocker(ctx, config) {
  ...
  this.ctx = ctx;
  this.config = config;
  this.originalPath = ctx.request.url;
...
Mocker.prototype.getFilename = function () {
  return `${this.config.dir}/${encodeURIComponent(this.path)}.${this.ctx.request.method}`;
};
...
Mocker.prototype.send = function (backend) {
  var path = this.ctx.request.path;
